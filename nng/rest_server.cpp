...
rest_get_job(void)
{
	...
	nng_mtx_unlock(job_lock);
	if ((job = static_cast<rest_job*>( calloc(1, sizeof(*job)) )) == NULL) {
		return (NULL);
	}
...
{
	rest_job *job = static_cast<rest_job*>( arg );
	nng_aio * aio = job->aio;
	int       rv;
...
{
	struct rest_job *job;
	nng_http_req *   req  = static_cast<nng_http_req *>( nng_aio_get_input(aio, 0) );
	nng_http_conn *  conn = static_cast<nng_http_conn *>( nng_aio_get_input(aio, 2) );
	const char *     clen;
	size_t           sz;
	nng_iov          iov;
	int              rv;
...
inproc_server(void *arg)
{
	...
	for (;;) {
		...
		body = static_cast<char*>( nng_msg_body(msg) );
